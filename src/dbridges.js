...
class dBridges
{
    ...
    access_token(callback)
	{
        ...
        if(!this.#dispatch.isExists("dbridges:access_token")){
    ...
    _accesstoken_dispatcher(channelName ,  action ,  response )
    {
        ...
        if(this.#dispatch.isExists("dbridges:access_token")){
            this.#dispatch.emit2("dbridges:access_token" , channelName ,  this.sessionid , action ,  response );
        }else{
    ...
    connect = async()=>{
      ...
      if(this.#retryCount == 0  && !this.connectionstate.get_newLifeCycle()){
        ...
        if (!jdata) {
            let db = new dBError("E008" , [this.auth_url]);
        ...
        case MessageTypes.CF_CALLEE_QUEUE_EXCEEDED:
            ...
            case MessageTypes.CONNECT_TO_RPC_SERVER:
                       ...
                       case MessageTypes.RPC_CALL_RESPONSE:
                           let mmpayload = "";
                          try {
                              mmpayload = (payload)? payload.toString(): "";
                          } catch (error) {
                              mmpayload = "";
                          }
          
                        rpccaller = this.rpc.get_object(sid)
                          rpccaller._handle_callResponse (sid ,mmpayload,rspend, rsub);
                          break;
    ...
    #IOConnectFailed  = (info)=>
    {
        ...
        this.connectionstate._handledispatcher(states.ERROR);
